...
async def predict(file: UploadFile = File(...)):
    """Возвращает детекции, удобные метрики (0–100), image_size и превью с боксами."""
    ...
    cls_part = min(30, unique_classes * 6)  # 5 классов => 30
    details = clamp(round(box_part + cls_part))

    plotted = results[0].plot()  
    img_bytes = io.BytesIO()
    Image.fromarray(plotted[..., ::-1]).save(img_bytes, format="PNG")  # BGR->RGB
